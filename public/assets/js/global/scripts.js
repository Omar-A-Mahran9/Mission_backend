...
let blockAlert = function (message = '') {
    if (message == '')
        message = `${__('Are you sure you want to block this') + ' ' + __('user') + ' ' + __('?') + ' ' + __('All data related to this') + ' ' + __('user') + ' ' + __('will be blocked')}`
    return Swal.fire({
        text: message,
        icon: "warning",
        showCancelButton: true,
...
let activateAlert = function (message = '') {
    if (message == '')
        message = `${__('هل أنت متأكد من الغاء حظر هذا') + ' ' + __('المستخدم') + ' ' + __('؟')}`
    return Swal.fire({
        text: message,
        icon: "warning",
        showCancelButton: true,
...
let confirmationAlert = function (message = __('Are You sure for doing this action ?'), icon = 'warning', confirmBtnText = __('Yes, Sure !'), cancelButtonText = __('No, Cancel')) {
...
let successAlert = function (message = __("Operation done successfully"), timer = 1000) {
...
function onImgError(image, placeholder = "/assets/dashboard/media/placeholders/default.png") {

...
var KTDatatablesServerSide = function () {
    ...
    var initDatatable = function () {
        ...
        if (categoryType === "sub") // if sub models then add nes column for the parent category name
    ...
    `;
                ...
                <div class="category-item m-2">
                    <span class="badge badge-light-info fs-7">${category.name ?? '__'}</span>
                </div>
            `;
                    ...
                    render: function (data) {
                        return `
                            <div>
                                <!--begin::Info-->
                                <div class="d-flex flex-column justify-content-center">
                                    <a href="javascript:;" class="mb-1 text-gray-800 text-hover-primary">${data ?? '__'}</a>
                                </div>
                                <!--end::Info-->
                            </div>
                        `;
                    ...
                    render: function (data, type, row) {
                        ...
                        <div>
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                ...
                                <!--end::Menu item-->
                                ${categoryType != 'parent' ? `<!--begin::Menu item-->
                                    <div class="menu-item px-3">
    ...
    var handleEditRows = () => {
        ...
        editButtons.forEach(d => {
            ...
            d.addEventListener('click', function (e) {
                ...
                $("#meta_tag_key_description_inp").val(data.meta_tag_key_description);
                if (categoryType == 'sub') {
                    if (data.size_applicable == 1) {
                        $("#kt_modal_update_address_billing").prop('checked', true);
                    } else {
                        $("#kt_modal_update_address_billing").prop('checked', false);
                    }
                    const categoryIds = data.categories.map(category => category.id);
                    $(`[name='parent_id[]']`).val(categoryIds).attr('selected', true);
                    $(`[name='parent_id[]']`).trigger('change');
                }
                if (categoryType == 'parent') {
                    if (data.tax_applicable == 1) {
                        $("#kt_modal_update_address_billing").prop('checked', true);
                    } else {
                        $("#kt_modal_update_address_billing").prop('checked', false);
                    }
                ...
                $("#crud_form").attr('action', `/dashboard/${dbTable}/${data.id}?type=${categoryType}`);

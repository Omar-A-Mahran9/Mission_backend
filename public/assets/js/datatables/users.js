...
var KTDatatablesServerSide = function () {
            ...
            columnDefs: [
                    ...
                    render: function (data, type, row, meta) {
                        return `
                            <div class="form-check form-check-sm form-check-custom form-check-solid" style="justify-content: center;">
                                <span class="mb-1 text-gray-600">${meta.row + 1}</span>
                            </div>`;
                    }
                },
                {
                ...
                },
                {
                    ...
                    render: function (data) {
                        let status = {
                            '1': { 'color': 'primary', 'title': __('Active') },
                            '2': { 'color': 'danger', 'title': __('Inactive') },
                        };
    ...
    var handleBlockingRows = () => {
        ...
        const blockingButtons = document.querySelectorAll('[data-kt-docs-table-filter="status_row"]');
        ...
        blockingButtons.forEach(d => {
            ...
            d.addEventListener('click', function (e) {
                ...
                if (data.status === 1) {
                    loadingAlert(__("InActive..."))
                } else {
                    loadingAlert(__("Active..."))

                }
                $.ajax({
                    type: "get",
                    url: `/dashboard/${dbTable}/status/${data.id}`,
                    data: {
                        status: data.status
                    },
                    success: function () {

                        ...
                        <div class="card-header">
                            ...
                            <div class="d-flex flex-xl-row gap-10 align-items-center">
                                ...
                                @if ($vendor->approved === App\Enums\VendorStatusEnum::Approved->value)
                                    <form class="reset-form" data-vendor-id="{{ $vendor->id }}">
                                        @csrf
                                        <button type="button" class="btn btn-primary btn-sm me-lg-n7 reject-btn">
                                            {{ __('Reset password') }}
                                        </button>
                                    </form>
                                    <form class="vendor-status-form" data-vendor-id="{{ $vendor->id }}"
                                ...
                                @endif
                                @if ($vendor->approved === App\Enums\VendorStatusEnum::Blocking->value)
                                    <form class="vendor-status-form" data-vendor-id="{{ $vendor->id }}"
                                        data-status="{{ App\Enums\VendorStatusEnum::Pending->value }}">
                                        @csrf
                                        <button type="button" class="btn btn-danger btn-sm me-lg-n7 reject-btn">
                                            {{ __('Unblocked') }}
                                        </button>
                                ...
                                @endif
                                @if ($vendor->approved === App\Enums\VendorStatusEnum::Rejected->value)
                                    <form class="vendor-status-form" data-vendor-id="{{ $vendor->id }}"
                                        data-status="{{ App\Enums\VendorStatusEnum::Pending->value }}">
                                        @csrf
                                        <button type="button" class="btn btn-primary btn-sm me-lg-n7 reject-btn"
                                    ...
                                    <tbody class="fw-semibold text-gray-600">
                                        ...
                                        <tr>
                                            <td class="text-muted">
                                                <div class="d-flex align-items-center">
                                                    <i
                                                        class="ki-outline ki-phone fs-2 me-2"></i>{{ __("Owner's Phone Number") }}
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            <td class="text-muted">
                                                <div class="d-flex align-items-center">
                                                    <i
                                                        class="ki-outline ki-phone fs-2 me-2"></i>{{ __('Branch Contact Number') }}
                                                </div>
                                            </td>
                                            <td class="fw-bold text-end">{{ $vendor->branch_number ?? 'ــ' }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-muted">
                                                <div class="d-flex align-items-center">
                                    ...
                                    <div
                                        ...
                                        <a class="d-block overlay" data-action="preview_commercial_register_image"
                                            ...
                                            <div class="symbol symbol-150px symbol-circle mb-7">
                                                <img src="{{ $vendor->commercial_register_image_path }}"
                                                    alt="image" />
                                            </div>
                                        </a>
                                    </div>
                        ...
                        <div class="d-flex flex-column gap-7 gap-lg-10">
                            <div class="d-flex flex-column flex-xl-row gap-7 gap-lg-10">
                                ...
                                <div class="card card-flush py-4 flex-row-fluid position-relative flipped">
                                    ...
                                    <div
                                        class="position-absolute top-0 end-0 bottom-0 opacity-10 d-flex align-items-center me-5">
                                        <a class="d-block overlay" data-action="preview_iban" href="#">
                                            <div class="symbol symbol-150px symbol-circle mb-7">
                                                <img src="{{ $vendor->iban_path }}" alt="image" />
                                            </div>
                                        </a>
                                    </div>
                                    <!--end::Background-->
...
@push('scripts')
    ...
    <script>
        ...
        const previewIbanPathButton = $('[data-action="preview_iban"]');
        ...
        $(previewIbanPathButton).on('click', function(e) {
            e.preventDefault();
            ...
            $(".attachments").append(`
            <!--begin::Overlay-->
            <a class="d-block overlay" data-fslightbox="lightbox-basic" href="${ibanPath}">
                <!--begin::Action-->
                <div class="overlay-layer card-rounded bg-dark bg-opacity-25 shadow">
                    <i class="bi bi-eye-fill text-white fs-3x"></i>
                </div>
    ...
    <script>
        $(document).ready(function() {
            $('.vendor-status-form button').on('click', function() {
                ...
                if (status == 0) {
                    BlockAlert("Are you sure you want to unblock this vendor?", function() {
                        changeVendorStatus(vendorId, status, token);
                    });
                } else if (status == 3) {
                    BlockAlert("Are you sure you want to block this vendor?", function() {
                        changeVendorStatus(vendorId, status, token);
                    });
                } else if (status == 2) {
                    BlockAlert("Are you sure you want to block this vendor?", function() {
            ...
            $('.reset-form button').on('click', function() {
                ...
                const status = form.data('status');
                const token = form.find('input[name="_token"]').val();
                BlockAlert("Are you sure you want to reset the password for this vendor?", function() {
                    $.ajax({
                        url: "{{ route('dashboard.vendor.reset', ':vendor_id') }}".replace(
                            ':vendor_id', vendorId),
                        method: 'POST',
                        data: {
                            _token: token,
        ...
        function changeVendorStatus(vendorId, status, token) {
            $.ajax({
                url: "{{ route('dashboard.vendor.change.status', ':vendor_id') }}".replace(':vendor_id', vendorId),
                method: 'POST',
                data: {
                    _token: token,
                    status: status
